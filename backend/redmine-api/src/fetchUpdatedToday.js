 ...
 */
export async function fetchIssuesUpdatedTodayByUser(targetUserId, projectId, config = {}) {
  const baseUrl = config.baseUrl || process.env.REDMINE_BASE_URL;
  if (!baseUrl) throw new Error("REDMINE_BASE_URL is not set");
  
  const apiKey = config.apiKey || process.env.REDMINE_API_KEY;
  if (!apiKey) throw new Error("REDMINE_API_KEY is not set");

  const basicUser = config.basicUser || process.env.BASIC_USER || "";
  const basicPass = config.basicPassword || process.env.BASIC_PASSWORD || "";
